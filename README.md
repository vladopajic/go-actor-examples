...
To make the most out of these examples, read each example package found in the [/example](/example/) directory. Every example has a bootstrap.go file that sets up the logic and explains the idea behind the example. It is also advised to read other source files in the example directory (package).
...
| 11 | Demonstrates how to create a custom actor (HTTPService) and seamlessly compose it with other actors. |
